...
public class BoardDataDrawer : Editor
{
    ...
    {
        InitializeReoedableList(ref dataList, "SearchWords", "Searching Words");
    }
    ...
    private void InitializeReoedableList(ref ReorderableList list, string propertyName, string listLabel)
    {
        ...
        if (GUILayout.Button("Fill Up With Random"))
        {
            for (int i = 0; i < GameDataInstance.Columns; i++)
            {
                for (int j = 0; j < GameDataInstance.Rows; j++)
                {
                    ...
                    string letters = "АБВГДЕЁЖЗИЙКЛМНТУФХЦЧШЩЪЫЬЭЮЯ";
